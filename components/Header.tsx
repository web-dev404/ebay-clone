		...
		<div className={'max-w-6xl mx-auto p-2'}>
			<nav className={'flex items-center justify-between'}>
				<div className={'flex items-center space-x-2 text-sm'}>
					{address ? (
						<button onClick={disconnect}
						        className={'connectWalletBtn'}>Hi, {address.slice(0, 5) + '...' + address.slice(-4)}</button>
					) : (
				...
				<div className={'flex items-center space-x-4 text-sm'}>
					...
					<Link className={'flex items-center hover:link'} href={'/addItem'}>Add to inventory <ChevronDownIcon
						className={'h-4'}/></Link>
			...
			<section className={'flex items-center space-x-2 py-5'}>
				<div className={'h-16 w-16 sm:w-20 md:w-44 cursor-pointer flex-shrink-0'}>
					<Link href={'/'}>
